...
REM --- Build proper connection string (host/port:full_path) ---
set DB_CONN=%DB_SERVER%/%DB_PORT%:%DB_FILE%

REM --- If database file does not exist, create it ---
if not exist "%DB_FILE%" (
    echo [!] Database not found-creating "%DB_FILE%"...

    REM 1) Generate temporary SQL for CREATE DATABASE
    (
    ...
    REM 2) Execute creation script via isql
    "%ISQL%" -u %DB_USER% -p %DB_PASS% -i "%~dp0_create_db.sql" 
    if ERRORLEVEL 1 (
        echo [!] Error creating database. Check create_db.sql
        exit /b 1
    )
    ...
    if ERRORLEVEL 1 (
        echo [!] Error applying schema. Check init_db.sql
        exit /b 1
    )
